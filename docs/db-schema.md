...
| `provider` | TEXT | CHECK IN ('toss','kakao'), NOT NULL | |
| `provider_user_id` | TEXT | NOT NULL | 토스 `userKey`(숫자지만 TEXT 저장 — 앱 단위 고유, [apps-in-toss.md](apps-in-toss.md) §4) / 카카오 봇 user id |
| `created_at` | TIMESTAMP | NOT NULL DEFAULT now | |
